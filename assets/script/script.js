$(document).ready(function () {
  ...
  $("#pollSearchBtn").on("click", function (e) {
    ...
    var NewQueryURLVoterInfo =
      queryBaseURL + "voterinfo" + APIkey + "&address=" + enteredAddress;
  ...
  function pollingQuery(queryURLVoterInfo) {
    ...
    $.ajax({ url: queryURLVoterInfo, method: "GET" }).then(function (response) {
      ...
      $("#elWell").append(
        "<h2>Election: <br>" + response.election.name + "</h2><br>"
      );
      $("#elWell").append(
        "<h2>Election Day: <br>" + response.election.electionDay + "</h2>"
      );
      ...
      for (var p = 0; p < response.pollingLocations.length; p++) {
        ...
        $("#pollWell-" + p).append(
          "<h2>Location : <br>" +
            response.pollingLocations[p].address.locationName +
            "<br>" +
            response.pollingLocations[p].address.line1 +
            ", " +
            response.pollingLocations[p].address.city +
            ", " +
            response.pollingLocations[p].address.state +
            ", " +
            response.pollingLocations[p].address.zip +
            "</h2><br>"
        );
        $("#pollWell-" + p).append(
          "<h2>Polling Hours: <br>" +
            response.pollingLocations[p].pollingHours +
            "</h2>"
        );
    ...
    $(document).ajaxError(function () {
      ...
      $("#elWell").append(
        "<p>Currently, there is no election information available! This might be due to the fact that there is no election in the near future, due to missing information in the Google Civic Information API or an error in your entered address.</p>"
      );
      ...
      $("#pollWell").append(
        "<p>Currently, there is no polling location information available! This might be due to the fact that there is no election in the near future, due to missing information in the Google Civic Information API or an error in your entered address.</p>"
      );
    });
  }
  ///candidate function
  ...
  function candidateQuery(queryURLVoterInfo) {
    ...
    $(document).ajaxError(function () {
      ...
      //append error message
      $("#candWell").append(
        "<p>Currently, there is no candidate information available! This might be due to the fact that there is no election in the near future, due to missing information in the Google Civic Information API or an error in your entered address.</p>"
      );
    });
  }
  ...
  function earlyVoteQuery(queryURLVoterInfo) {
    ...
    $.ajax({ url: queryURLVoterInfo, method: "GET" }).then(function (response) {
      ...
      for (var e = 0; e < response.earlyVoteSites.length; e++) {
        ...
        $("#earlyWell-" + e).append(
          "<h2>Location : <br>" +
            response.earlyVoteSites[e].address.locationName +
            "<br>" +
            response.earlyVoteSites[e].address.line1 +
            ", " +
            response.earlyVoteSites[e].address.city +
            ", " +
            response.earlyVoteSites[e].address.state +
            ", " +
            response.earlyVoteSites[e].address.zip +
            "</h2><br>"
        );
        $("#earlyWell-" + e).append(
          "<h2>Polling Hours: <br>" +
            response.earlyVoteSites[e].pollingHours +
            "</h2><br>"
        );
        $("#earlyWell-" + e).append(
          "<h2>Start Date: <br>" +
            response.earlyVoteSites[e].startDate +
            "</h2><br>"
        );
        $("#earlyWell-" + e).append(
          "<h2>End Date: <br>" + response.earlyVoteSites[e].endDate + "</h2>"
        );
    ...
    $(document).ajaxError(function () {
      ...
      //append error message
      $("#earlyWell").append(
        "<p>Currently, there is no early vote information available! This might be due to the fact that there is no election in the near future, due to missing information in the Google Civic Information API or an error in your entered address.</p>"
      );
    });
  }
  ...
  function runRepresentativeQuery(queryURLRepresentatives) {
    ...
    $.ajax({ url: queryURLRepresentatives, method: "GET" }).then(function (
      response
    ) {
      ...
      for (var i = 0; i < response.officials.length; i++) {
        ...
        var imgUrl =
          response.officials[i].photoUrl || "http://placehold.it/128x128";

        //Attach content to approp well
        $("#repWell-" + i).append(
          "<div class='card-image mx-auto'><figure class='image is-128x128'><img src=" +
            imgUrl +
            "></figure></div>"
        );
        $("#repWell-" + i).append(
          "<div class='card-content mx-auto'><div class='content>"
        );
        $("#repWell-" + i).append(
          "<h4>Office: " + response.offices[i].name + "</h4>"
        );

        $("#repWell-" + i).append(
          "<h4>Name: " + response.officials[i].name + "</h4>"
        );
        $("#repWell-" + i).append(
          "<h4>Party: " + response.officials[i].party + "</h4>"
        );

        var offUrl =
          response.officials[i].urls && response.officials[i].urls[0]
            ? response.officials[i].urls[0]
            : "no url available";
    ...
    $(document).ajaxError(function () {
      ...
      //append error message
      $("#repWell").append(
        "<p>No information available! This might be due to the missing information in the API (e.g. the state not updating the information) or an error in your entered address.</p>"
      );
    });
  }
});
